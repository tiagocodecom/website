...
pub fn Container(
    ...
    #[prop(
        default = "section bg-white dark:bg-boxDark rounded-lg px-6 py-8 md:px-8 md:py-10 lg:p-12 shadow-sectionBoxShadow hover:shadow-sectionBoxShadowHover transition ease-out duration-[160ms]"
    )]
    class: &'static str,
) -> impl IntoView {
    view! {
...
#[component]
pub fn ContainerSubtitle(
    children: Children,
    #[prop(default = "")] backdrop_text: &'static str,
) -> impl IntoView {
    view! {
        <span
            data-backdrop-text=backdrop_text
...
pub fn ContainerTitle(children: Children) -> impl IntoView {
    view! { <h2 class="text-4xl font-poppins font-semibold mb-2 dark:text-white">{children()}</h2> }
}

#[component]
pub fn ContainerDescription(children: Children) -> impl IntoView {
    view! { <p class="text-pColor dark:text-white/70">{children()}</p> }
}
