...
pub fn TimelineItem(date: String, title: String, subtitle: String) -> impl IntoView {
    view! {
        <div class="group">
            ...
            <h3 class="font-poppins font-medium text-lg lg:text-xl mt-2 mb-1 lg:mt-3 lg:mb-2 dark:text-white">
                {title.clone()}
            </h3>
            <span class="text-pColor dark:text-white/70">{subtitle.clone()}</span>
        </div>
    }
}
