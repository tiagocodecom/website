...
pub fn BasicLayout(children: Children) -> impl IntoView {
    ...
    view! {
        <Transition fallback=move || { view! { <div class="bg-main"></div> }}>
            <header class="fixed w-full z-20 top-0 start-0 bg-white border-b border-gray-200" id="header">
                {move || {
                    layout
                    .get_untracked()
                    .map(|data| {
            ...
            <footer class="bg-main text-center py-8 text-sm text-gray-500">
